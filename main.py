...
def load_app_icon() -> Optional[QtGui.QIcon]:
    """Load the app icon (captureIcon.ico) with PyInstaller support."""
    try:
...
def load_splash_logo_pixmap(size: int) -> Optional[QtGui.QPixmap]:
    """Load the splash logo from programLogo.ico if available."""
    try:
        base = Path(sys._MEIPASS) if getattr(sys, "frozen", False) else Path.cwd()
    except Exception:
        base = Path.cwd()
...
def main() -> None:
    ...
    scaled_logo = load_splash_logo_pixmap(174)
    logo_y = 100
    text_y = 120
    if scaled_logo is not None:
